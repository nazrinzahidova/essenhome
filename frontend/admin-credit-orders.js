  ...
  async function load(){
    ...
    try{
      ...
      for(const order of data.items){
        ...
        const info=el('dl');
        for(const [label,value] of [['Müraciət №',order.id],['Tarix',new Date(order.createdAt).toLocaleString('az-AZ')],['Ad',order.firstName],['Soyad',order.lastName],['Ata adı',order.fatherName],['Telefon',order.phone],['FIN kod',order.fin],['SİMA',order.hasSima?'Var':'Yoxdur']]) info.append(el('dt',label),el('dd',value));
