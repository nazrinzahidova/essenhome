...
for(const [file,category,fn,count,split] of [['washingmachine','Paltaryuyan maşınlar','washingMachineSpecEntries',33,'Yuma zamanı yükləmə imkanı'],['fridge','Soyuducular','fridgeSpecEntries',30,'Qapı istiqamətinin dəyişdirilməsi'],['freezer','Dondurucular','freezerSpecEntries',27,'Kompressor tipi']])test(category+' fields and edit round trip',()=>{
 ...
 ctx.preservedProductSpecs=saved;inputs[0].value='Boz';inputs[1].value='';const edited=ctx.mergeCategorySpecs();assert.equal(edited.Rəng,'Boz');assert(!('Zəmanət' in edited));assert.equal(edited.SKU,'keep');
 const rows=ctx[fn]({brand:'Bosch',specs:edited});assert.equal(rows.length,count);assert.equal(rows[0][1],'Bosch');assert.equal(rows[Math.ceil(count/2)][0],split);
